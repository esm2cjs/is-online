...
replace_imports "p-any" # to = "@esm2cjs/FROM"
replace_imports "p-timeout" # to = "@esm2cjs/FROM"
replace_imports "public-ip" # to = "@esm2cjs/FROM"
...
PJSON=$(cat package.json | jq --tab '
	...
	| .xo = {ignores: ["cjs", "test.mjs", "**/*.test-d.ts", "**/*.d.ts"]}
	| .ava.files = ["test.mjs"]

	| .dependencies["@esm2cjs/got"] = .dependencies["got"]
	| del(.dependencies["got"])
	| .dependencies["@esm2cjs/p-any"] = .dependencies["p-any"]
	| del(.dependencies["p-any"])
	| .dependencies["@esm2cjs/p-timeout"] = .dependencies["p-timeout"]
	| del(.dependencies["p-timeout"])
	| .dependencies["@esm2cjs/public-ip"] = .dependencies["public-ip"]
	| del(.dependencies["public-ip"])
')
